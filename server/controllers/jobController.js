...
function buildPublicCompanyProfile(profile, recruiterUser = {}) {
  ...
  const profileApproved = String(profile?.approvalStatus || '').toLowerCase() === 'approved';
  const userApproved = String(recruiterUser?.status || '').toLowerCase() === 'approved';

  const contactParts = [];
  if (profile?.preferredContactMethod) contactParts.push(`Preferred: ${profile.preferredContactMethod}`);
  if (profile?.allowCandidateMessages === 'Yes') contactParts.push('Candidate messages allowed');
  if (profile?.averageResponseTime) contactParts.push(`Response: ${profile.averageResponseTime}`);

  return {
    companyLogoUrl: profile?.companyLogoUrl || '',
    companyName: profile?.companyName || recruiterUser?.companyName || recruiterUser?.name || '',
...
exports.getPublicJobById = async (req, res) => {
  try {
    ...
    const recruiterProfile = await RecruiterProfile.findOne({ user: recruiterId }).lean();
    const jobObj = job.toObject();

    // Only public-safe company profile fields are attached for candidates.
    // Internal verification documents, reviewedBy and rejectionReason are not exposed.
    jobObj.companyProfile = buildPublicCompanyProfile(recruiterProfile, job.createdBy);
